...
//    }
    ...
    private static List<MealTo> filteredByAtomic(List<Meal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        return meals.stream()
                .peek(meal -> caloriesSumByDate.merge(meal.getDate(), meal.getCalories(), Integer::sum))
                .peek(meal -> {
                    exceededMap.computeIfAbsent(meal.getDate(), date -> new AtomicBoolean());
                    exceededMap.get(meal.getDate()).set(caloriesSumByDate.get(meal.getDate()) > caloriesPerDay);
                })
                .filter(meal -> isBetweenHalfOpen(meal.getTime(), startTime, endTime))
                .map(meal -> createTo(meal, exceededMap.get(meal.getDate())))
                .collect(Collectors.toList());
    }
...
//    }
//
    private static MealTo createTo(Meal meal, AtomicBoolean excess) {
